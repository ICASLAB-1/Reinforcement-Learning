...
def episode_start(env, model, prev_obs, epsilon, not_move_list):
    ...
    while True:
        ...
        append_sample(memory, preprocess(prev_obs), action, reward, preprocess(obs), done)
        prev_obs = obs
...
for i in range(max_episodes):
    ...
    thrd1 = Process(target=episode_start, args=((1, 2), virtual1_env, model, virtual1_prev_obs, epsilon, not_move_list))
    thrd2 = Process(target=episode_start, args=((3, 4), virtual2_env, model, virtual2_prev_obs, epsilon, not_move_list))
    thrd3 = Process(target=episode_start, args=((5, 6), virtual3_env, model, virtual3_prev_obs, epsilon, not_move_list))
    thrd4 = Process(target=episode_start, args=((7, 8), virtual4_env, model, virtual4_prev_obs, epsilon, not_move_list))
