...
env_list = [virtual1_env, virtual2_env, virtual3_env, virtual4_env]
gamma = 0.9
batch_size = 512
...
model = Multi_Network()
torch.save(model.state_dict(), 'target4.pth')
target_model = Multi_Network()
target_model.load_state_dict(torch.load('target4.pth'))
...
torch.save(model.state_dict(), '2048_virtual4_original_normal.pth')
df1 = pd.DataFrame(scores)
df2 = pd.DataFrame(steps)
df3 = pd.DataFrame(max_tile)
df1.to_csv("virtual4_scores_train.csv")
df2.to_csv("virtual4_steps_train.csv")
df3.to_csv("virtual4_max_tile_train.csv")
