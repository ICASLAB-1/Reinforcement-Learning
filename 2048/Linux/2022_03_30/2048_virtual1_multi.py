...
model = Multi_Network()
torch.save(model.state_dict(), 'target1.pth')
target_model = Multi_Network()
target_model.load_state_dict(torch.load('target1.pth'))
...
torch.save(model.state_dict(), '2048_virtual1_original_normal.pth')
df1 = pd.DataFrame(scores)
df2 = pd.DataFrame(steps)
df3 = pd.DataFrame(max_tile)
df1.to_csv("virtual1_scores_train.csv")
df2.to_csv("virtual1_steps_train.csv")
df3.to_csv("virtual1_max_tile_train.csv")
