...
virtual5_env = gym.make('2048-v0')
virtual6_env = gym.make('2048-v0')
virtual7_env = gym.make('2048-v0')
virtual8_env = gym.make('2048-v0')
virtual9_env = gym.make('2048-v0')

env_list = [virtual1_env, virtual2_env, virtual3_env, virtual4_env, virtual5_env, virtual6_env, virtual7_env,
            virtual8_env, virtual9_env]
gamma = 0.9
batch_size = 512
min_memory = batch_size*8
memory = []
dense = 128
...
def train_model(model, target_model, optim, cri, memory):
    np.random.shuffle(memory)
    length = len(memory) // batch_size
...
model = Multi_Network()
torch.save(model.state_dict(), 'target9.pth')
target_model = Multi_Network()
target_model.load_state_dict(torch.load('target9.pth'))
...
for i in range(max_episodes):
    ...
    while True:
        ...
        if epsilon > epsilon_min and iteration % 20 == 0:
    ...
    if len(memory) >= min_memory:
        train_model(model, target_model, optimizer, criterion, memory)
        memory = []
...
torch.save(model.state_dict(), '2048_virtual9_original_normal.pth')
df1 = pd.DataFrame(scores)
df2 = pd.DataFrame(steps)
df3 = pd.DataFrame(max_tile)
df1.to_csv("virtual9_scores_train.csv")
df2.to_csv("virtual9_steps_train.csv")
df3.to_csv("virtual9_max_tile_train.csv")
