...
def preprocess(obs):
    x = np.zeros([layer_count, 4, 4]) # 4X4 의 observation space, 14개의 타일 종류
    for k in range(12):
        matching = pow(2, k)
        for i in range(4):
            for j in range(4):
...
model = Multi_Network()
torch.save(model.state_dict(), 'target0.pth')
target_model = Multi_Network()
target_model.load_state_dict(torch.load('target0.pth'))
...
for i in range(max_episodes):
    ...
    #     torch.save(model.state_dict(), '2048_virtual0_original_best.pth')

    print(i, 'score: ', score, 'step: ', step, 'max tile: ', np.max(main_obs), 'memory len: ', len(memory), 'epsilon: ', epsilon)

torch.save(model.state_dict(), '2048_virtual0_original_normal.pth')
df1 = pd.DataFrame(scores)
df2 = pd.DataFrame(steps)
df3 = pd.DataFrame(max_tile)
df1.to_csv("virtual0_scores_train.csv")
df2.to_csv("virtual0_steps_train.csv")
df3.to_csv("virtual0_max_tile_train.csv")
